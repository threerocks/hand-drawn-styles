...
4. 风格 1.2 的正式生产、多页或连续故事必须完整消费渲染器 JSON 中的 `workflow`:基础生成使用 `prompt` 与 `references`,随后执行必需的 `scribble-correction`;基础图不可作为 final,任一阶段缺失时停止生产。
5. 其他画风默认只输出最终 prompt 纯文本;风格 1.2 正式生产输出 `family-crayon-card-v2` JSON 调用包。**不要生图**。
