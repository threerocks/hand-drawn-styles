...
- `family-crayon-card-v2` 是强制两阶段合同。第一阶段按顶层 `prompt` + `references` 生成基础图,它的状态只能是 `intermediate-only`;第二阶段必须按 `workflow.stages[id=scribble-correction]` 做编辑,把基础图作为 Image 1、同一画风锚点作为 Image 2,完整使用阶段 prompt。只有第二阶段输出才允许进入 final。
- 第二阶段只修正蜡笔涂抹轨迹,必须保留第一阶段的人物、五官、姿势、动作、构图、物件数量、黑色轮廓、配色、白底和画幅。业务项目不得自己改写第二阶段 prompt,也不得因为基础图“看着还行”而跳过它。
...
例外:风格 1.2 用于正式生产、连续故事或多页作品时,输出正式 JSON 调用包,调用端必须完整执行 `workflow` 两阶段;只复制 prompt、丢掉画风锚点或跳过 `scribble-correction`,都属于不合格调用,不能产出 final。
