...
4. 风格 1.2 用于正式生产、连续故事或多页作品时,必须完整执行渲染器 JSON 的 `workflow`:先用 `prompt` 与 `references` 生成基础图,再执行必需的 `scribble-correction` 编辑阶段;基础图只能算中间产物。锚点或第二阶段不可用就停止正式生产。
5. 其他画风默认只输出最终 prompt;风格 1.2 的正式生产默认输出 `family-crayon-card-v2` JSON 调用包,纯文本只允许显式 `--text-only-preview`。不生图;仓库维护者新增或验收画风时,按 `AGENTS.md` 的维护者验证例外执行。
