...
每种画风一个具体示例:**样图 + 可直接复制的输入示例提示词**。下列 19 种画风 + 3 变体(共 22 段)配方均已用真实出图验证通过。
...
![亲子投稿蜡笔故事卡画风锚点](../assets/style-1.2/anchor-family.png)

> 输入示例:「用 1.2 画爸爸把零食袋放回柜子,男孩站在旁边看着,不加文字」
> 正式生产、连续故事和多页作品必须把 `assets/style-1.2/anchor-family.png` 作为纯画风参考随每一张请求传入。使用 `scripts/render_prompt.py --format json` 可同时取得完整 prompt 和锚点绝对路径。
